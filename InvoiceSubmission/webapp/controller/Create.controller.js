sap.ui.define([
	...
	return Controller.extend("ns.InvoiceSubmission.controller.Create", {
        ...
        processPDF : function(file)
        {
			...
			xhr.open("POST", "/<<app router path>>/doxDest/document/jobs");
            xhr.send(data);
        },
        waitComplete : function(requestID,waitingCalls){
			...
			$.ajax({
				type: "GET",
				url: "/<<app router path>>/doxDest/document/jobs/"+requestID,
				cache: false,
				dataType: "json",
				async: false,
        ...
        },sendRequestCreate: function(itemsLength,header,items)
        {
            ...
            if(counterCalls == 0)
            {
                url = "/<<app router path>>/ProcurementService/browse/Invoice";
                payload = header;
            }
            else
            {
                url = "/<<app router path>>/ProcurementService/browse/InvoiceItems";
            ...
            $.ajax({
                ...
                xhr.open("POST", "/<<app router path>>/DocManagement/browser/<<Your Repository ID>>/root/Invoices");
                xhr.send(data);
            },   
